...
Map<String, Object?> _req(int id, String method, [Map<String, Object?>? params]) {
...
void main() {
  test('initialize echoes protocol version and advertises tools', () async {
    final r = await _server().handle(_req(1, 'initialize', {
      'protocolVersion': '2025-06-18',
      'capabilities': <String, Object?>{},
    }));
    final result = r!['result'] as Map<String, Object?>;
    expect(result['protocolVersion'], '2025-06-18');
    expect((result['capabilities'] as Map<String, Object?>).containsKey('tools'), isTrue);
    expect((result['serverInfo'] as Map<String, Object?>)['name'], 'gpsphototag');
  });

  test('notifications (no id) get no response', () async {
    final r = await _server()
  ...
  test('tools/list returns the catalog with input schemas', () async {
    ...
    final tools = (r!['result'] as Map<String, Object?>)['tools'] as List<Object?>;
    ...
    for (final t in tools) {
      expect((t as Map<String, Object?>)['inputSchema'], isA<Map<String, Object?>>());
    }
  });

  test('tools/call check_toolkit returns content + structuredContent', () async {
    final r = await _server()
        .handle(_req(3, 'tools/call', {'name': 'check_toolkit', 'arguments': {}}));
    final result = r!['result'] as Map<String, Object?>;
  ...
  test('tools/call validates arguments (no photos -> tool error)', () async {
    final r = await _server().handle(
        _req(4, 'tools/call', {'name': 'tag_photos', 'arguments': {'photos': <String>[]}}));
    final result = r!['result'] as Map<String, Object?>;
    expect(result['isError'], isTrue);
    expect((result['structuredContent'] as Map<String, Object?>)['code'], 'bad_input');
  });

  test('unknown tool -> invalid params error', () async {
    final r = await _server()
        .handle(_req(5, 'tools/call', {'name': 'nope', 'arguments': {}}));
  ...
  test('get_capabilities reflects exiftool availability', () async {
    final off =
        McpServer(tools: buildTools(runner: _FakeRunner(), exiftoolAvailable: false));
    final r = await off
        .handle(_req(7, 'tools/call', {'name': 'get_capabilities', 'arguments': {}}));
    final structured =
        ((r!['result'] as Map<String, Object?>)['structuredContent']) as Map<String, Object?>;
    expect(structured['exiftool_available'], isFalse);
    expect((structured['formats'] as Map<String, Object?>)['heic'],
        contains('exiftool'));
  });
}
