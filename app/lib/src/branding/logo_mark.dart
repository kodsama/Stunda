...
class _LogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final pin = Path()
      ...
      ..arcToPoint(const Offset(82, 40),
          radius: const Radius.circular(32), clockwise: true)
    ...
    // Lens disc.
    canvas.drawCircle(
        const Offset(50, 40), 22, Paint()..color = AppColors.paperRaised);

    // Tiny landscape clipped to the lens.
    canvas.save();
    canvas.clipPath(Path()..addOval(Rect.fromCircle(center: const Offset(50, 40), radius: 21)));
    canvas.drawRect(const Rect.fromLTWH(29, 19, 42, 42),
        Paint()..color = const Color(0xFFCFE3DD));
    canvas.drawRect(const Rect.fromLTWH(29, 42, 42, 19),
        Paint()..color = const Color(0xFFEAF0EC));
    canvas.drawCircle(
        const Offset(42, 31), 5, Paint()..color = const Color(0xFFE0A33A));
    canvas.drawPath(
      Path()
        ..moveTo(29, 52)
        ..lineTo(45, 33)
